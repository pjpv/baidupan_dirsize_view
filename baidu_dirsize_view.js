    ...
    */
	function clear_cache() {
		...
		for (var key in localStorage) {
			if (key.substring(0, 7) == 'bdsize_') localStorage.removeItem(key);
		}
	}
...
*/
    ...
    * 每一行都是独立事件
    */
	$('.zJMtAEb .NHcGw').bind('DOMNodeInserted',
	function(e) {
		if (e.target.nodeName === 'DD') {
			...
			if (html) {
				...
				$(e.target).find('.file-name').next().html(html).css({
					"width": "16%",
					"overflow": "hidden",
	...
	$(document).ready(function(e) {
		...
		for (var i = 0; i < $all_dd.length; i++) {
			...
			if (html) {
				...
				$($all_dd[i].target).find('.file-name').next().html(html).css({
					"width": "16%",
					"overflow": "hidden",
					"box-shadow": "inset 0 1px 2px rgba(0, 0, 0, 0.1)",
					//                                "border-radius": "4px",
...
*/
    ...
    */
	function setFileSize(total) {
		...
		for (var i = 0; i < $items.length; i++) {
			...
			percentage + '%' + '</span><span class="file-info" style="">' + size_str + '</span>';

			$($items[i]).html(html).css({
				"width": "16%",
				"overflow": "hidden",
				"box-shadow": "inset 0 1px 2px rgba(0, 0, 0, 0.1)",
				//                "border-radius": "4px",
			...
			function(res) {
				...
				if (res && res.errno === 0 && res.status === 'success') {
					...
					for (var i = 0; i < res.list.length; i++) {
						...
						percentage + '%' +
						//</span><span class="file-info" style="float:left;position:absolute;display:block;text-shadow:0-1px0rgba(0,0,0,0.25);">' +
						'</span><span class="file-info" style="">' + '大小：' + size + ' | 目录数：' + item_result.dirnum + ' | 文件数：' + item_result.filenum + '</span>';

						$(item_obj.obj).parent().parent().next().html(html).css({
							"width": "16%",
							"overflow": "hidden",
							"box-shadow": "inset 0 1px 2px rgba(0, 0, 0, 0.1)",
							//                                "border-radius": "4px",
						...
						if (i > 0 || is_home) localStorage.setItem(key_prefix + utf8_to_b64(item_obj.path), html);
					}
					// 设置文件大小视图
					setFileSize(total);
				}
    ...
    */
	function filterFile() {
		...
		var t = new Date();
		$('.DxdbeCb form[method="get"] input').bind('input propertychange blur',
		function(e) {
